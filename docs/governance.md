...
`policy-check` compares policy with the same target revision used for coverage: thresholds cannot decrease, source roots cannot shrink, exclusions cannot be added, and baseline floors/expiry cannot be weakened. A new exclusion requires an explicit policy migration, not an ordinary code change. Record baseline start and expiry; the maximum interval is 30 days.
