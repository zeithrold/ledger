...
--
-- River job-queue schema, exported from the pinned River release so that goose
-- stays the single explicit migration entrypoint. River's migration line
-- "main" versions 2-7 are included; version 1 only creates River's private
-- migration-tracking table and is intentionally excluded because Ledger owns
-- migrations and the River client never reads that table at runtime.
--
-- River version: v0.47.0
--
-- Regeneration pipeline (run from the repository root):
--   go run github.com/riverqueue/river/cmd/river@v0.47.0 migrate-get \
--     --line main --all --exclude-version 1 --up   > /tmp/river_up.sql
--   go run github.com/riverqueue/river/cmd/river@v0.47.0 migrate-get \
--     --line main --all --exclude-version 1 --down > /tmp/river_down.sql
-- then split each file into dollar-quote-aware statements and wrap every
-- statement in goose statement-block annotations, so the semicolon splitter
-- never breaks a plpgsql or SQL function body. When River is upgraded, export
-- only versions above 7 into a new goose migration; never rewrite an applied
-- migration.
--
-- This file must not run inside a single transaction: River migration 006
-- creates an immutable function that depends on the enum change made by
-- migration 004, which PostgreSQL rejects in the same transaction.
